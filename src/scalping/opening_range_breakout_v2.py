    ...
    for index, row in data.iterrows():
        ...
        if position > 0:
            if use_trailing_stop:
                current_stop = max(initial_stop_loss, row["Close"] - (trailing_stop_factor * row_atr))
            else:
        ...
        # Exit conditions if in a position
            ...
            if stop_hit or profit_target_hit or sell_signal:
                ...
                trades.append({"Action": "SELL", "Index": index, "Price": exit_price, "Profit": profit, "Balance": balance})
...
def main():
    ...
    print("Optimized backtesting complete. Results saved to orb_strategy_optimized_results.json.")
